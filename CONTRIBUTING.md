...
Release a package before the packages that depend on it. The plugin and `@frsource/playwright-visual-regression-diff` depend on `@frsource/visual-regression-manifest` as `workspace:^`, which `pnpm publish` rewrites to a caret range on the version the manifest package has in the repository at that moment (`"@frsource/visual-regression-manifest": "^1.0.0-beta.1"`). Two consequences:

- A plugin (or Playwright package) release PR merged while the manifest package still carries the `0.0.0` placeholder (or a version that is not on npm yet) publishes a package that cannot be installed (`^0.0.0` matches nothing on the registry). Merge the manifest package's release PR first, wait for its publish, then merge the dependents'. Do not let both land in one combined release PR: `scripts/publish.mjs` publishes in `pnpm -r ls` order, which is alphabetical by directory, so the plugin would go out before the manifest package it depends on. `release-please-config.json` does not set `separate-pull-requests`, so release-please may bundle several packages into one release PR; check the release PR before merging it.
